...
function fixWinptyCc() {
	...
	if (
		...
		console.log(`ℹ️  Already patched: ${WINPTY_CC_PATH}`);
		return false;
	}
...
`;
	...
	fixedFunction = fixedFunction.replace(
		/\bwinpty_error_ptr_t error_ptr = nullptr;/g,
		"",
	);
	fixedFunction = fixedFunction.replace(
		/\bwinpty_config_t\* winpty_config = /g,
		"winpty_config = ",
	);
	fixedFunction = fixedFunction.replace(/\bwinpty_t \*pc = /g, "pc = ");
	fixedFunction = fixedFunction.replace(
		/\bwinpty_spawn_config_t\* config = /g,
		"config = ",
	);
	fixedFunction = fixedFunction.replace(/\bHANDLE handle = nullptr;/g, "");
	fixedFunction = fixedFunction.replace(
		/\bBOOL spawnSuccess = /g,
		"spawnSuccess = ",
	);
	fixedFunction = fixedFunction.replace(
		/\bv8::Local<v8::Object> marshal = /g,
		"marshal = ",
	);

	// Insert early declarations
	fixedFunction =
		fixedFunction.substring(0, insertPoint) +
		earlyDeclarations +
		fixedFunction.substring(insertPoint);
